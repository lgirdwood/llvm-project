...
private:
  ...
  bool isStandaloneHiFiInstr(const MCInst &MI, StringRef Name,
                             const MCSubtargetInfo &STI) const;
  void getSelect_256OpValue(const MCInst &MI, unsigned OpNo, APInt &Value, SmallVectorImpl<MCFixup> &Fixups, const MCSubtargetInfo &STI) const;
};
} // namespace
...
bool XtensaMCCodeEmitter::isStandaloneHiFiInstr(const MCInst &MI, StringRef Name,
  ...
  if (Name.starts_with("AE_MOVDA32") || Name.starts_with("AE_MOVDA16")) {
    ...
    IsStandalone = StringSwitch<bool>(Name)
      ...
      .StartsWith("AE_MULAAFD32X16_H3_L2_HIFI3", true)
      .StartsWith("AE_MULAAFD32X16_H1_L0_HIFI3", true)
      .StartsWith("AE_MULAF32R_HH_HIFI3", true)
      .StartsWith("AE_MULAF32R_LL_HIFI3", true)
      .StartsWith("AE_MULAF32R_LH_HIFI3", true)
      .StartsWith("AE_MULAAFD32RA_HH_LL_HIFI3", true)
      .StartsWith("AE_MULAF32S_LH_HIFI3", true)
      .StartsWith("AE_MULAF32S_LL_HIFI3", true)
      .StartsWith("AE_MULAF32S_HH_HIFI3", true)
  ...
  if (Opcode == Xtensa::BUNDLE || (Name.starts_with("AE_") && !isStandaloneHiFiInstr(MI, Name, STI))) {
    ...
    if (SubInsts.size() == 1) {
      ...
      if (isStandaloneHiFiInstr(Sole, SoleName, STI)) {
        // FLIX_MAC_Inst instructions need a dedicated 6-byte Format 0 MAC
        // bundle encoding. Their TableGen encoding is an 8-byte XtensaInst64
        // that does NOT represent a valid standalone packet; the correct HW
        // encoding is { NOP; MAC_op; NOP } packed into 6 bytes.
        unsigned FLIXMACOpcode = StringSwitch<unsigned>(SoleName)
        ...
        if (FLIXMACOpcode) {
          unsigned t = Ctx.getRegisterInfo()->getEncodingValue(
              Sole.getOperand(0).getReg());
          unsigned s = Ctx.getRegisterInfo()->getEncodingValue(
              Sole.getOperand(2).getReg());
          unsigned r = Ctx.getRegisterInfo()->getEncodingValue(
              Sole.getOperand(3).getReg());

          uint32_t val0 = 0x27b205;  // NOP for slot 0 in MAC bundle
          uint32_t val1 = (FLIXMACOpcode << 12) | (s << 8) | (r << 4) | t;

          uint32_t insn0 = 0x0e, insn1 = 0x2000;
          // Format 0 slot 0 packing
          insn0 = (insn0 & ~0xf00u) | ((val0 & 0xf) << 8);
          insn0 = (insn0 & ~0xf0u) | (((val0 & 0xf0) >> 4) << 4);
          insn0 = (insn0 & ~0xf000u) | (((val0 & 0xf00) >> 8) << 12);
          insn0 = (insn0 & ~0xf0000000u) | (((val0 & 0xf000) >> 12) << 28);
          insn1 = (insn1 & ~0x3fu) | ((val0 & 0x3f0000) >> 16);
          // Format 0 slot 1 packing
          insn0 = (insn0 & ~0xf0000u) | ((val1 & 0xf) << 16);
          insn0 = (insn0 & ~0xf000000u) | (((val1 & 0xf0) >> 4) << 24);
          insn0 = (insn0 & ~0xf00000u) | (((val1 & 0xf00) >> 8) << 20);
          insn1 = (insn1 & ~0x3fc0u) | (((val1 & 0xff000) >> 12) << 6);
    ...
    auto encodeSlotInstr = [&](const MCInst &SlotInst, unsigned SlotIdx) -> uint32_t {
      ...
      auto getRegNum = [&](const MCOperand &MO) -> unsigned {
        if (!MO.isReg()) return 0;
        const MCRegisterInfo *MRI = Ctx.getRegisterInfo();
        return MRI->getEncodingValue(MO.getReg());
      };

      if (Opc == Xtensa::L8UI || Opc == Xtensa::L16UI || Opc == Xtensa::L16SI || Opc == Xtensa::L32I) {
        unsigned t = getRegNum(SlotInst.getOperand(0));
        unsigned s = getRegNum(SlotInst.getOperand(1));
        int64_t imm = SlotInst.getOperand(2).getImm();
        unsigned scale = 1;
        if (Opc == Xtensa::L16UI || Opc == Xtensa::L16SI) scale = 2;
        else if (Opc == Xtensa::L32I) scale = 4;
        unsigned imm8 = (imm / scale) & 0xff;
        unsigned prefix = (SlotIdx == 0) ? 0x17 : 0x07;
        if (Opc == Xtensa::L16UI) prefix = (SlotIdx == 0) ? 0x15 : 0x05;
        else if (Opc == Xtensa::L16SI) prefix = (SlotIdx == 0) ? 0x14 : 0x04;
        else if (Opc == Xtensa::L32I) prefix = (SlotIdx == 0) ? 0x16 : 0x06;
        Val = (prefix << 16) | (imm8 << 8) | (t << 4) | s;
        return Val;
      }

      if (Opc == Xtensa::S8I || Opc == Xtensa::S16I || Opc == Xtensa::S32I) {
        unsigned t = getRegNum(SlotInst.getOperand(0));
        unsigned s = getRegNum(SlotInst.getOperand(1));
        int64_t imm = SlotInst.getOperand(2).getImm();
        unsigned scale = 1;
        if (Opc == Xtensa::S16I) scale = 2;
        else if (Opc == Xtensa::S32I) scale = 4;
        unsigned imm8 = (imm / scale) & 0xff;
        unsigned prefix = 0x1a;
      ...
      if (Opc == Xtensa::ADDI) {
        unsigned t = getRegNum(SlotInst.getOperand(0));
        unsigned s = getRegNum(SlotInst.getOperand(1));
        unsigned imm8 = SlotInst.getOperand(2).getImm() & 0xff;
        unsigned prefix = (SlotIdx == 0) ? 0x12 : 0x02;
        Val = (prefix << 16) | (imm8 << 8) | (t << 4) | s;
        return Val;
      }
      ...
      if (Opc == Xtensa::MOVI) {
        unsigned t = getRegNum(SlotInst.getOperand(0));
        unsigned imm12 = SlotInst.getOperand(1).getImm() & 0xfff;
        unsigned prefix = (SlotIdx == 0) ? 0x1b : 0x08;
        Val = (prefix << 16) | (((imm12 >> 4) & 0xff) << 8) | (t << 4) | (imm12 & 0xf);
        return Val;
      }

      if (Opc == Xtensa::ADD || Opc == Xtensa::SUB || Opc == Xtensa::AND || Opc == Xtensa::OR || Opc == Xtensa::XOR ||
          Opc == Xtensa::ADDX2 || Opc == Xtensa::ADDX4 || Opc == Xtensa::ADDX8) {
        unsigned dest = getRegNum(SlotInst.getOperand(0));
        unsigned s = getRegNum(SlotInst.getOperand(1));
        unsigned t = getRegNum(SlotInst.getOperand(2));
        unsigned prefix = (SlotIdx == 0) ? 0x1c : 0x0a;
        unsigned opcExt = 4;
        if (Opc == Xtensa::SUB) {
          prefix = (SlotIdx == 0) ? 0x1d : 0xb;
          opcExt = (SlotIdx == 0) ? 3 : 6;
        } else if (Opc == Xtensa::AND) {
          opcExt = (SlotIdx == 0) ? 9 : 0xb;
        } else if (Opc == Xtensa::OR) {
          prefix = (SlotIdx == 0) ? 0x1d : 0xb;
          opcExt = (SlotIdx == 0) ? 2 : 4;
        } else if (Opc == Xtensa::XOR) {
          prefix = (SlotIdx == 0) ? 0x1d : 0xb;
          opcExt = (SlotIdx == 0) ? 7 : 0xa;
        } else if (Opc == Xtensa::ADDX2) {
          opcExt = 5;
        } else if (Opc == Xtensa::ADDX4) {
          opcExt = 6;
        } else if (Opc == Xtensa::ADDX8) {
          opcExt = 7;
        }
        Val = (prefix << 16) | (opcExt << 12) | (dest << 8) | (t << 4) | s;
        return Val;
      }

      if (Opc == Xtensa::NEG) {
        unsigned dest = getRegNum(SlotInst.getOperand(0));
        unsigned t = getRegNum(SlotInst.getOperand(1));
        unsigned prefix = (SlotIdx == 0) ? 0x26 : 0x0f;
        unsigned opcExt = (SlotIdx == 0) ? 8 : 3;
        unsigned val_s = (SlotIdx == 0) ? dest : t;
        Val = (prefix << 16) | (opcExt << 12) | (dest << 8) | (t << 4) | val_s;
        return Val;
      }
      ...
      if (Opc == Xtensa::EXTUI) {
        unsigned dest = getRegNum(SlotInst.getOperand(0));
        unsigned s = getRegNum(SlotInst.getOperand(1));
        unsigned shift = SlotInst.getOperand(2).getImm() & 0xf;
        unsigned len = SlotInst.getOperand(3).getImm();
        unsigned opcExt = (len - 1) << 1;
        unsigned prefix = (SlotIdx == 0) ? 0x10 : 0x00;
        prefix |= (opcExt >> 4) & 1;
        Val = (prefix << 16) | ((opcExt & 0xf) << 12) | (dest << 8) | (s << 4) | shift;
        return Val;
      }
      ...
      if (Opc == Xtensa::SEXT) {
        unsigned dest = getRegNum(SlotInst.getOperand(0));
        unsigned s = getRegNum(SlotInst.getOperand(1));
        unsigned prefix = (SlotIdx == 0) ? 0x1d : 0x0d;
        unsigned opcExt = (SlotIdx == 0) ? 9 : 10;
        Val = (prefix << 16) | (opcExt << 12) | (dest << 8) | s;
        return Val;
      }

      if (Opc == Xtensa::SLLI || Opc == Xtensa::SRLI || Opc == Xtensa::SRAI) {
        unsigned dest = getRegNum(SlotInst.getOperand(0));
        unsigned s = getRegNum(SlotInst.getOperand(1));
        unsigned shift = SlotInst.getOperand(2).getImm() & 0xf;
        unsigned prefix = (SlotIdx == 0) ? 0x1c : 0x0a;
        unsigned opcExt = 1;
        if (Opc == Xtensa::SRLI) {
          prefix = (SlotIdx == 0) ? 0x24 : 0x0e;
          opcExt = 1;
        } else if (Opc == Xtensa::SRAI) {
          prefix = (SlotIdx == 0) ? 0x1c : 0x0a;
          opcExt = 2;
        }
        if (Opc == Xtensa::SLLI) {
          unsigned shift_val = (32 - shift) & 0xf;
          Val = (prefix << 16) | (opcExt << 12) | (dest << 8) | (shift_val << 4) | s;
        } else {
          Val = (prefix << 16) | (opcExt << 12) | (dest << 8) | (s << 4) | shift;
        }
        return Val;
      }
