...
bool XtensaRegisterInfo::eliminateFrameIndex(MachineBasicBlock::iterator II,
  ...
  if (!MI.isDebugValue() && !Valid) {
    ...
    // Create a virtual register for the offset; PEI's frame-index scavenging
    // (enabled by requiresFrameIndexScavenging) resolves it to a physical
    // register afterwards with correct liveness.
